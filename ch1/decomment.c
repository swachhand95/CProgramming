    ...
    while ((c = getchar()) != EOF) {
        if (c == '\'' && !in_single_quote && prev != '\\') {
            ...
            while (!is_illegal_state(state) && !is_final_state(state) && (c = getchar()) != EOF) {
                ...
                /* Get the new state from the NFA table */
            ...
            if (is_final_state(state)) {
                ...
                if (nl) {
                    /* Print the number of new lines encountered in comment */
                    int i;
                    for (i = 0; i < nl; ++i)
                        putchar('\n');
                    nl = 0;
                    ...
                    /* Print a space if it was a single line comment */
                    putchar(' ');
                }
                /* Clear the buffer */
                buf_clear();
                ...
                /* It isn't a valid comment
                   Print the characters that were processed by NFA till now
        ...
        /* Used to detect the '\' of escape sequences */
        prev = c;
    }
